...
<div  class="bg-sky-200 w-full h-72">
    <div class="container h-full flex gap-14 items-center">
        ...
        <div class="p-2 w-3/5">
            ...
            <div>{{ optional($praxis->text)->freitext_aboutus }}</div>
        </div> 
    </div>
</div>
...
<div class="container flex gap-5">
    <div class="basis-3/4 px-1 mt-16">    
        <div class="mb-20">
            {{ optional($praxis->text)->freitext_one }}
        </div>
        <div class="mb-24">
        ...
        <div>
            @foreach($praxis->clinicServices as $service)
                <div class="font-semibold">{{ $service->service->service_title }}</div>       
                <div class="mb-4">{{ $service->service_text }}</div>
            @endforeach
        </div>
        <div>

            <!-- <div class="pb-3">{{ optional($praxis->serviceOthers)->other_one }}</div>
            <div class="pb-3">{{ optional($praxis->serviceOthers)->other_two }}</div> -->
        </div>
    </div>
