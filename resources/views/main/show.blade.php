...
<div class="container flex gap-14">
    <div class="px-1 mt-16 basis-3/4">  

        <section class="mb-20">
            @include('components.praxis.praxis-text-block', ['title' => 'title_one', 'text' => 'text_one'])
            @include('components.praxis.praxis-text-block', ['title' => 'title_two', 'text' => 'text_two'])
            @include('components.praxis.praxis-text-block', ['title' => 'title_three', 'text' => 'text_three'])
        </section>

        <section>
            <h3 class="font-bold mb-6 text-[1.4rem] text-sky-800">Fachbereiche</h3>
            <x-praxis.praxis-therapy-list :category="1" :praxis="$praxis" />
        </section>
        <section class="mt-24">
            <h3 class="font-bold mb-6 text-[1.4rem] text-sky-800">Leistungen</h3>
            <x-praxis.praxis-therapy-list :category="2" :praxis="$praxis" />
        </section> 
    </div>


    <div class="basis-1/4 px-4 bg-slate-100 bg-opacity-50 text-sm">
        <div class="mt-12 space-y-1">
            ...
            @if(isset($praxis->tel))
                <div class="pt-6 flex items-center my-2 gap-2">
            ...
            <div class="flex items-center my-2 gap-2">
                ...
                {{ optional(isset($praxis->user))->email }}
            </div>
            @endif
            @if(isset($praxis->website))
                <div class="flex items-center my-2 gap-2">
        ...
        <div class="mt-24">
            ...
            <div style="line-height: 1.8em;"> {!! nl2br(e($praxis->text->text_sitebar )) !!}</div>
        </div>
    </div>
</div>
@endsection
